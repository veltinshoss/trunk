...
import monkeypuzzle.io.parser.sqlite.SqlDataSource;
import monkeypuzzle.io.parser.sqlite.dynamicproxy.Where.Expression;
import monkeypuzzle.util.FormattedArrayList;
...
public class SqlDynamicProxy<T> extends DynamicProxy<T>
{
	...
	public static <S> S loadRootData(final Class<S> interfaceDef,
			...
			SQLException
	{
		final List data = loadData((Class) ((ParameterizedType) interfaceDef
			...
			throws SqlMappingException, SQLException
	{
		...
		Table table = Table.createTable(interfaceDef, inputWhere);
			...
			throws Throwable
	{
		...
		} else
		{
			...
			Object result = this.rs.getObject(name);
