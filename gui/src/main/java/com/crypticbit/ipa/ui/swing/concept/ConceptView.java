...
public class ConceptView extends ViewingPane {
	...
	public ConceptView(final Mediator mediator) {
		...
		final EventList events = mediator.getBackupDirectory().getAllEvents(
				ProgressIndicator.nullProgressIndicator);
		final ConceptDataModel conceptTableModel = new ConceptDataModel(events);

		final JComponent mapPanel = new ConceptWherePanel(mediator,
				conceptTableModel);
		final JComponent table = new ConceptTable(mediator, conceptTableModel);
		final JComponent whatPane = new ConceptWhatPanel(mediator,
				conceptTableModel);
		final JComponent whenPane = new ConceptWhenPanel(mediator,
				conceptTableModel);
		final JComponent itemPane = new ConceptItemPanel(mediator,
				conceptTableModel);

		panels = new ConceptPanel[] { (ConceptPanel) mapPanel,
				(ConceptPanel) table, (ConceptPanel) whatPane,
				(ConceptPanel) whenPane, (ConceptPanel) itemPane };
		...
		final JSplitPane selectPanel = new JSplitPane(
...
//		addTab("Report", new ConceptReport(mediator, conceptTableModel));

	}

	@Override
	public void cleanUp() {
		// don't need to unregister listeners as not global
	}
}
