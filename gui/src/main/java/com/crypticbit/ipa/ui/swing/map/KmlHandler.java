...
public class KmlHandler
{
	...
	public JButton createOpen()
	{
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				try
				{
					temp = new File(com.crypticbit.ipa.io.util.IoUtils
							.createTempDir(".deviceRoot"), "open.kml");
					outputToKml(temp);
	...
	public void outputToKml(File file) throws FileNotFoundException
	{
		...
		final Document document = kml.createAndSetDocument()
				.withName("iPhoneAnalzyer-export.kml").withOpen(true);
		...
		for (KmlWaypoint waypoint : waypointProvider.getWaypoints())
		{
			...
			if (color != null)
			{
				...
				if (!seen.contains(color))
				{
					...
					style.withId("style_" + color.hashCode())
							// set the stylename to use this style from the
							// placemark
							.createAndSetIconStyle()
							.withColor(
									"FF" + asHex(color.getBlue())
											+ asHex(color.getGreen())
											+ asHex(color.getRed()))
							.withScale(1.0);
				}
			}
			...
			final Placemark placemark = document.createAndAddPlacemark()
					...
					.withDescription(waypoint.getDescription());
			if (color != null)
				placemark.withStyleUrl("#style_" + color.hashCode());

			final Point point = placemark.createAndSetPoint();
			List<Coordinate> coord = point.createAndSetCoordinates();

			coord.add(new Coordinate(waypoint.getLongitude(), waypoint
					.getLatitude(), 0));
		}
		kml.marshal(file);
