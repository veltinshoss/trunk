...
public class EventImpl implements Event {
	...
	private Map<Tag, WhoImpl> who = new HashMap<Tag, WhoImpl>() {
		...
		public String toString() {
			...
			if (size() == 1)
				...
				return size() + " people (" + super.toString() + ")";
		}
	};
	private String what;
	boolean empty = true;
	...
	public enum Concept {
		WHEN(WhenTag.class) {
			@Override
			public void addToEvent(ConceptFactory conceptFactory, EventImpl event,
					Object annotation, Object value, Tag tagPrefix, Object object) {
				...
				if (value != null) {
					...
					if (!event.when.containsKey(tag))
						event.when.put(tag, new WhenImpl());
		...
		WHERE(WhereTag.class) {
			...
			public void addToEvent(ConceptFactory conceptFactory, EventImpl event,
				...
				if (value != null) {
					...
					if (!event.where.containsKey(tag))
						event.where.put(tag, new WhereImpl());
						...
						try {
							...
							if (result != null) {
								if (result instanceof Field)
									field = (Field) result;
								if (result instanceof WhoConceptable)
									field = ((WhoConceptable) result)
											.getConceptType();
							}
						} catch (Exception e) {
							LogFactory.getLogger().log(Level.WARNING,
									"Problem processing concepts", e);
						}
		...
		public abstract void addToEvent(ConceptFactory conceptFactory, EventImpl event,
				Object annotation, Object value, Tag tagPrefix, Object object);
	}

	public void add(Concept concept, Object annotation, Object value,
			Tag tagPrefix, Object object) {
		if (value != null) {
