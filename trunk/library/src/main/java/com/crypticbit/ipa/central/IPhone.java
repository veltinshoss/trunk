...
public class IPhone {
			...
			try {
			    ...
			    LogFactory
				    ...
				    .log(Level.WARNING,
					    "Unable to scan "
						    + i.getName()
						    + " for concepts - probably because it's of an unknown version: "
			    ...
			    LogFactory
				    ...
				    .log(Level.SEVERE,
					    "Unable to scan "
						    + i.getName()
						    + " for concepts - probably because of an application error: ",
					    e);
			}
		}
	    }
	...
	if (bfv != null) {
		...
		LogFactory.getLogger().log(
			Level.WARNING,
			"Ignoring badly formed file ("
				+ getBackupFileByType(bfv)
				+ ") - which doesn't meet the criteria for "
				+ clazz.getName());
		return null;
	    }
	} else
